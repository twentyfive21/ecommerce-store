...
const setList = () => {
    let list = getLocalStorage(); // Get the cart items from local storage
    if (list.length > 0) {
        list.forEach(cartItem => {
            const item = items.find(item => item.id === cartItem.id); // Find the corresponding item in the items array
            if (item) {
                createItem({ ...cartItem, ...item }); // Create the item row with both cart and item data
            }
        });
    } else {
        createNullItem();
...
// Load items on DOMContentLoaded
...
const createItem = ({ id, quantity, productName, price, image }) => {
    ...
    newRow.innerHTML = `
        ...
        <td class="subtotal-cart-price hide-for-mobile">
            $${(parseFloat(price.slice(1)) * quantity).toFixed(2)}
        </td>
    ...
    deleteButton.addEventListener('click', () => {
        removeItemFromLocalStorage(id); // Remove from local storage
        newRow.remove(); // Remove the row from the table
        calculateTotal(getLocalStorage()); // Recalculate total
    });
}
...
const clearLocalStorage = () => {
    localStorage.setItem('cartList', JSON.stringify([])); // Clear the cart
}
...
const calculateTotal = (itemArray) => {
    ...
    for (let i = 0; i < itemArray.length; i++) {
        subtotal += (parseFloat(itemArray[i].price.slice(1)) * itemArray[i].quantity); // Calculate subtotal
    }

    shipping = subtotal === 0 ? 0.00 : 12.99; // Calculate shipping
    shippingPrice.innerText = `$${shipping.toFixed(2)}`; // Update shipping price
    const total = subtotal + shipping; // Total calculation
    subtotalBox.innerText = `$${subtotal.toFixed(2)}`; // Update subtotal display
    totalBox.innerText = `$${total.toFixed(2)}`; // Update total display
}
...
checkoutBtn.addEventListener("click", () => {
    jsConfetti.addConfetti({
        confettiRadius: 5,
        confettiNumber: 500,
        confettiColors: ["#151412", "#fff674", "#ffbec2", "#c8d6ff"],
    });
});
