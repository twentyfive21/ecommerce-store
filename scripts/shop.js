...
window.onload = function () {
    ...
    const selectedCategory = urlParams.get("c"); // = get category
    
    if (selectedCategory) {
        displaySelectedCategory(selectedCategory);
    } else {
        displayItems(items); // Display all items on page load
    }
};
...
const displaySelectedCategory = (category) => {
    let filteredItems = items; // Start with all items
    ...
    if (category === 'Mens') {
        ...
        filteredItems = items.filter(item => item.category === "Accessories");
    } else if (category === 'Shoes') {
        filteredItems = items.filter(item => item.category === "Shoes");
    } else if (category === 'Jewelry') {
        filteredItems = items.filter(item => item.category === "Jewelry");
    }
...
const displayFilteredItems = (value) => {
    ...
    newArray = items.filter(item => {
        const price = parseFloat(item.price.slice(1)); // Assuming price is in the format "$xx.xx"
